...
""" A Slurm ResumeProgram to create OpenStack instances.
    ...
    The flavor, image, network and keypair to be used must be defined as node Features [3] in the format "parameter=value".
    ...
    DNS must be available and `SlurmctldParameters=cloud_dns` set in the slurm.conf [4].
    ...
    [4]: https://slurm.schedmd.com/slurm.conf.html#OPT_cloud_dns
    ...
    for node in new_nodes:
        ...
        if not debug:
            ...
            with open(os.path.join(statedir, node), 'w') as f:
                f.write(server.id)
            # Don't need scontrol update nodename={node} nodeaddr={server_ip} as using SlurmctldParameters=cloud_dns
